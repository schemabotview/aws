...
this repo upgrades and re-verifies. The package ships the **official AWS service icon set**, so a node
still names one with `icon: 'ec2'` and this repo no longer depends on `aws-react-icons` itself.
